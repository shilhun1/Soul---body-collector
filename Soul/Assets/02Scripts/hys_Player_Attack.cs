...
public class hys_Player_Attack : MonoBehaviour
{
    ...
    // Bow는 지상에서 활을 쏘는 동안 자리를 잡고 수평 이동을 멈춥니다.
    public bool ShouldLockGroundMovementForBowAttack =>
        playerState != null &&
    ...
    private void TickAxeDiveAttack()
    {
        ...
        bool lostAxeBody = possessionSystem == null ||
            possessionSystem.CurrentWeaponType != HWJ_WeaponType.Axe;
    ...
    private void StartFirstAttack()
    {
        ...
        if (ShouldLockGroundMovementForBowAttack && rb != null)
        {
            // 달리다가 활을 쏴도 미끄러지지 않도록 공격 시작 순간 수평 속도를 제거합니다.
