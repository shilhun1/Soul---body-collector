...
public class HWJ_BossDialogueBubbleSystem : MonoBehaviour
{
    ...
    [SerializeField]
    private string introDialogue = "왔군.";

    [SerializeField]
    private string phaseTwoDialogue = "여기서 끝내겠다.";

    [SerializeField]
    private string soulLostDialogue = "도망칠 생각인가.";

    [SerializeField]
    private float defaultDurationSeconds = 2.2f;

    [SerializeField]
    private Color textColor = Color.black;

    [SerializeField]
    private Color bubbleColor = new Color(1f, 1f, 1f, 0.92f);

    [SerializeField]
    private int sortingOrder = 220;
    ...
    public void ShowLine(string line, float durationSeconds)
    {
        ...
        float width = Mathf.Clamp(
            line.Length * 0.18f + 0.8f,
            1.8f,
            4.6f
        );

        float height = Mathf.Clamp(
            Mathf.Ceil(line.Length / 14f) * 0.38f + 0.55f,
            0.8f,
            2.2f
        );
        ...
        hideRoutine = StartCoroutine(
            HideAfterSeconds(durationSeconds)
        );
    }
