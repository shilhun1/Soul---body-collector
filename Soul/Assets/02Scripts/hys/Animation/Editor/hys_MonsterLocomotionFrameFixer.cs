...
public static class hys_MonsterLocomotionFrameFixer
{
    ...
    private const string PlayerRunClipPath =
        "Assets/05Anims/hys_Player_Anims/Axe/Clips/hys_Player_Axe_Run.anim";

    private const int FrameCount = 8;
    // 보행 자세가 읽히도록 8프레임을 1초 동안 재생합니다.
    private const float FrameInterval = 0.125f;
    ...
    public static void Fix()
    {
        ...
        ApplyLocomotionClip(WalkClipPath, sprites);
        // 빙의 후 플레이어 조작에서는 별도의 Run 클립이 재생되므로 같은 프레임을 함께 적용합니다.
        ApplyLocomotionClip(PlayerRunClipPath, sprites);
        AssetDatabase.SaveAssets();
        AssetDatabase.Refresh();
        Debug.Log("[hys Locomotion] 몬스터 Walk와 빙의 플레이어 Run에 도끼 8프레임 적용 완료");
    }

    private static void ApplyLocomotionClip(string clipPath, Sprite[] sprites)
    {
        AnimationClip clip = AssetDatabase.LoadAssetAtPath<AnimationClip>(clipPath);
        if (clip == null)
        {
            throw new InvalidOperationException($"도끼 이동 클립을 찾을 수 없습니다: {clipPath}");
        }
        ...
        // 마지막 프레임도 1초까지 유지하여 반복 경계에서 프레임이 튀지 않게 합니다.
