...
def render_sword_skill_frames(skill: str) -> list[Image.Image]:
    ...
    sheet_path = SWORD_SKILL_SHEET_ROOT / f"hys_Sword_{skill}_sheet.png"
    ...
    for cell in cells:
        ...
        paste_x = round(50 - quantized.width / 2)
        paste_y = GROUND_Y - quantized.height
        canvas.alpha_composite(quantized, (paste_x, paste_y))
...
def build_skill(weapon: str, skill: str, sequence: list[SourcePose]) -> dict[str, object]:
    ...
    for index, pose in enumerate(sequence):
        if sword_frames is not None:
            ...
            source = str(
                (SWORD_SKILL_SHEET_ROOT / f"hys_Sword_{skill}_sheet.png").relative_to(ROOT)
            ).replace("\\", "/")
            edits = ["skill_specific_pose_sheet", "identity_palette_and_scale_lock"]
        else:
